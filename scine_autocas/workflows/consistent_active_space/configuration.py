...
class ConsistentActiveSpaceConfiguration:
    ...
    def __init__(self):
        ...
        """
        str
            Minimal basis for IAO/IBO construction. Available choices:

              MINAO    Default. cc-pVTZ (Dunning) for Z=1-36; ANO-RCC for Z>=37.
                       Sufficient for singlets of all elements.
              MINAO1   Tier-1 ANO-RCC for ALL Z (including Z=1-36). Use for doublets.
              MINAO2   Tier-2 ANO-RCC for ALL Z. Use for triplets.
              MINAO3   Tier-3 ANO-RCC for ALL Z. Use for quartets.

            MINAO uses the original Serenity minimal basis (cc-pVTZ) for light elements (Z=1-36)
            and ANO-RCC for heavy elements (Z>=37). MINAO1/2/3 use increasingly larger ANO-RCC
            bases for ALL Z — necessary for open-shell systems where nOcc.alpha > nMINAO/2.
            If nMINAO <= nOcc.alpha the workflow will abort; use a larger tier.
            Rule of thumb: MINAO(n) where n >= (spin_multiplicity - 1) / 2.
        """
        self.allow_zero_valence_virtuals: bool = False
        """
        bool
            If True, allow IBO localization to proceed with zero virtual valence orbitals
            (nMINAO <= nOcc). By default, autoCAS crashes with an explanatory error.
            Use a larger MINAO tier instead of this flag unless you have a specific reason.
        """
        ...
        """
        self.save_per_geom_dmrgscf: bool = False
        """
        bool
            If True, copy dmrgscf.h5 from dmrg/ to pergeom/ for each geometry after the
            DMRG entropy step. No new calculation — reuses the existing DMRG output.
            Lighter alternative to --save-per-geom-casscf that avoids convergence issues.
        """
    ...
    def get_serenity_interface_settings(self) -> Dict:
        ...
        if self.allow_zero_valence_virtuals:
            settings["Interface"]["allow_zero_valence_virtuals"] = True
